...
from ..nn.layers import clone_modules, LayerNorm2d
...
class SimpleMaskDownSampler(nn.Module):
    def __init__(
        ...
        in_ch, out_ch = multiplex_count, starting_out_chan
        for _ in range(num_layers):
            ...
            self.encoder.append(LayerNorm2d(out_ch))
            self.encoder.append(activation())
            in_ch = out_ch

        self.encoder.append(nn.Conv2d(out_ch, embed_dim, kernel_size=1))
