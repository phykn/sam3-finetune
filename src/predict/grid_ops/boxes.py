...
def scale_area(area, shape, crop):
    height, width = shape
    crop_x0, crop_y0, crop_x1, crop_y1 = crop
    scale_x = (crop_x1 - crop_x0) / width
    scale_y = (crop_y1 - crop_y0) / height
...
def filter_candidates(items, nms, max_masks):
    if not items:
        return []
    ...
    return [items[index] for index in keep[:max_masks]]
