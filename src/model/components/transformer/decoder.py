...
class TransformerDecoder(nn.Module):
    def __init__(
        ...
        self.fine_layers = (
            clone_modules(interaction_layer, num_layers)
    ...
    def _get_rpb_matrix(self, reference_boxes, feat_size):
        H, W = feat_size
        boxes_xyxy = convert_to_xyxy(reference_boxes).transpose(0, 1)
    ...
    def forward(
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            query_sine_embed = encode_sine_position(
            ...
            # iter update
            if self.box_refine:
                reference_before_sigmoid = invert_sigmoid(reference_boxes)
...
def functional_attention(
    ...
    *,
    ...
    if use_fa3:
        raise RuntimeError("FA3 attention is not included in the VLM-free src runtime")
    else:
