...
class GroundDec(nn.Module):
    ...
    def decode(self, enc, prompt, prompt_mask):
        ...
        hs, refs, _, presence_feats = self.transformer.decoder(
            tgt=tgt,
            memory=enc["memory"],
        ...
        self.score_and_box(out, hs, refs, prompt, prompt_mask)
        return out, hs

    def score_and_box(self, out, hs, refs, prompt, prompt_mask):
        apply_dac = self.transformer.decoder.dac and self.training
