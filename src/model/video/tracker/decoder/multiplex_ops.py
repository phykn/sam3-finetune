...
def project_mask_tokens(decoder, mask_tokens_out):
    ...
    for index in range(decoder.num_mask_output_per_object):
        token_index = 0 if decoder.decode_mask_with_shared_tokens else index
        hyper_in.append(
            decoder.output_hypernetworks_mlps[index](
                mask_tokens_out[:, :, token_index, :]
            )
        )
    return torch.stack(hyper_in, dim=2)


def predict_masks(decoder, hyper_in, upscaled_embedding):
...
def score_objects(decoder, obj_score_token_out, iou_pred):
    if decoder.pred_obj_scores:
        if (
            ...
            return (
                decoder.pred_obj_score_head(obj_score_token_out)
                .view(
                    iou_pred.shape[0],
                    decoder.multiplex_count,
                    decoder.num_mask_output_per_object,
        ...
        return decoder.pred_obj_score_head(obj_score_token_out)
