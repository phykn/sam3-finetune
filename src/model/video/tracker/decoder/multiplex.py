...
from ....components.nn.layers import LayerNorm2d
...
class MultiplexMaskDecoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        self._init_mask_layout(
            ...
            multimask_outputs_only=multimask_outputs_only,
            decode_mask_with_shared_tokens=decode_mask_with_shared_tokens,
    ...
    def predict_masks(
        ...
        masks = predict_masks(self, hyper_in, upscaled_embedding)

        iou_pred = self.iou_prediction_head(iou_token_out).view(
            b, self.multiplex_count, self.num_mask_output_per_object
        )
        object_score_logits = score_objects(self, obj_score_token_out, iou_pred)
        return {
            "masks": masks,
            "iou_pred": iou_pred,
