...
def box_iou_xyxy(box_a: Sequence[float], box_b: Sequence[float]) -> float:
    ...
    intersection = box_area_xyxy((ix0, iy0, ix1, iy1))
    union = box_area_xyxy(box_a) + box_area_xyxy(box_b) - intersection
    if union <= 0:
        return 0.0
...
def nms_boxes_xyxy(
    ...
    for index in order:
        ...
        if all(box_iou_xyxy(candidate, boxes[kept]) <= iou_threshold for kept in keep):
